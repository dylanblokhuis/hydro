...
pub fn compile_app(path: &Path, data: String) -> String {
    ...
    import_map.insert("react".into(), out_dir.join("react.js"));
    import_map.insert("react-dom".into(), out_dir.join("react-dom.js"));
    import_map.insert("$route".into(), Path::new("routes").join(path));
    import_map.insert("$lib".into(), out_dir.join("lib.tsx"));
    ...
    entries.insert(
        "main".to_string(),
        FileName::Real("./src/templates/entry.server.tsx".into()),
    );
...
impl Resolve for Resolver {
    fn resolve(&self, base: &FileName, target: &str) -> Result<FileName, Error> {
        ...
        if let Some(import_map) = self.import_map.get(target) {
            ...
            let file_path = cwd.join("src/templates").join(import_map);
            return self.resolve_as_file(&file_path).and_then(|p| self.wrap(p));
        }
